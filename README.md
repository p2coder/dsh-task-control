...
| 操作 | 行为 |
|---|---|
| **暂停** (Pause) | 立即中断正在进行的回合（同取消的 abort 语义，保留输入队列），把会话标记为已暂停，并记住被中断任务对应的原始用户提示词。若存在进行中的同会话 goal，会一并 `pause`，阻止 goal 自动轮次继续推进。 |
| **恢复** (Resume) | 清除暂停标记，把记住的原始提示词经 `Agent.followup()` 重新入队并唤醒驱动——agent 从已有历史（含被中断前的部分产出）继续执行该任务。 |
| **取消** (Cancel) | 中断正在进行的回合（保留输入队列），并清除暂停标记。与界面原有"停止"等价，只是也会清理暂停态。 |
