...
/** Safe-pause reasoning granularity fallback (kept for API compatibility). */
const DEFAULT_SAFE_PAUSE_REASONING = DEFAULT_SETTINGS.safeReasoning;
...
* state/settings routes. `config` is accepted for backward compatibility;
* pause defaults come from the durable settings store (default safe + wait).
*/
...
//#endregion
export { DEFAULT_SAFE_PAUSE_REASONING, DEFAULT_SETTINGS, apply, clearPaused, clearStoredState, createStateHandler, createTaskControlService, currentState, encodeSessionId, inject, markPaused, name, readSettings, readStoredState, settingsFilePath, stateFilePath, stateRootDir, writeSettings, writeStoredState };
