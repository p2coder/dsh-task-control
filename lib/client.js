window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/** Fetch JSON from the host (same-origin). */
		const settingsApi = (path, options) => fetch(path, options).then((res) => res.json());
		...
		*/
		function TaskControlSettingsPanel({ locale }) {
			...
			const load = react.useCallback(() => {
				settingsApi("/task-control/settings").then((res) => {
			...
			const save = async () => {
				...
				setNotice(null);
				try {
					const res = await settingsApi("/task-control/settings", {
						method: "POST",
