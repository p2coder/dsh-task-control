window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			"group.aria": "任务控制",
			"state.paused": "任务已暂停",
			"confirm.resumeForced": "工具 {tool} 在强制暂停前已被取消，可能已部分执行并产生副作用。确认后将从该工具调用重新执行。继续？",
			"action.pause": "暂停任务",
			"action.resume": "恢复任务",
			"action.cancel": "取消任务"
		};
		const en = {
			...
			"confirm.resumeForced": "Tool {tool} was cancelled during a force pause and may have partially executed with side effects. Confirm to re-execute from that tool call?",
		...
		*/
		function TaskControlDock({ useSession, useProjection, command, t }) {
			...
			const forced = projection !== void 0 && projection !== null && projection.forced === true;
			const interruptedTool = projection !== void 0 && projection !== null ? projection.interruptedTool : null;
			...
			const resume = () => {
				if (forced) {
					const toolName = interruptedTool?.name ?? "未知工具";
					if (!window.confirm(t("confirm.resumeForced", { tool: toolName }))) return;
					run("/resume confirm");
					return;
				}
				run("/resume");
